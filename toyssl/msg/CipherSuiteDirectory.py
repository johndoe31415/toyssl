...
class VerboseCipherSuite(object):
	_PSEUDO_SUITES = set([
		CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV,
		CipherSuite.TLS_FALLBACK_SCSV,
	])

	class _CSElements(object):
...
class CipherSuiteDirectory(object):
	...
	def filter_secure(self):
		...
		secure_ciphers = secure_ciphers.filter_sig_algorithm(lambda sig_algorithm: sig_algorithm.identifier in [ "ECDSA", "ECC" ])
	...
	def __str__(self):
		return "CipherSuiteDir<%d>" % (len(self))

if __name__ == "__main__":
	csdir = CipherSuiteDirectory().filter_secure()
	csdir.dump()
